...
// - периоды не пересекаются глубже одного общего месяца: общий пограничный
//   месяц — переход между работами (реально: Iktin до Dec 2025, Spetsvuz с Dec 2025),
...
`totalExperience` — единственная нетривиальная логика в модели, и она считает **объединение
интервалов с включительным счётом месяцев**: перерывы (реальный пробел Dec 2024 — Jun 2025)
не входят в стаж, а общий пограничный месяц двух ролей считается один раз. На реальных датах
резюме при asOf = Aug 2026 выходит 43 месяца = 3 года 7 месяцев.
...
4. ~~**Пересечения периодов**~~ → **решено (уточнено при реализации):** параллельных работ
   не будет, но общий пограничный месяц — норма (переход между работами). Конфликт =
   пересечение глубже одного месяца; `totalExperience` — объединение интервалов.
